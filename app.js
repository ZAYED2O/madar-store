...
async function loadAdminShipping() {
  try {
    ...
    data.governorates.forEach(g => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td style="font-weight:800;color:#000000">${g.name_ar}</td>
        <td style="font-weight:800;color:#000000">${g.name_en}</td>
        <td>
          <div style="display:flex;align-items:center;gap:0.8rem">
            <input type="number" class="gov-cost-input" data-id="${g.id}" value="${g.shipping_cost}" min="0" step="5"
              style="width:9rem;padding:0.6rem;border:1.5px solid #000000;border-radius:var(--r-sm);background:#ffffff;color:#000000;-webkit-text-fill-color:#000000;font-weight:800;font-size:1.4rem;text-align:center">
